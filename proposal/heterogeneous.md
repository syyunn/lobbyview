...
We need to leverage the [node embeddings](http://snap.stanford.edu/proj/embeddings-www/files/nrltutorial-part1-embeddings.pdf) since most of the actors in the ***Lobby Graph*** constitute a subgraph of ***Lobby Graph*** thus we can embed those subgraph's relations into a vector so that the vector represents the node's characteristics.
